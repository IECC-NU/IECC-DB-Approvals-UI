  ...
  try {
    ...
    for (let wtr of wtrRows) {
      const activities = await client.query(`
        ...
        ORDER BY log_id
      `, [wtr.coda_wtr_id]);
      wtr.activities = activities.rows;
    }
...
app.put('/api/wtr/:wtrId/status', async (req, res) => {
  try {
    ...
    const result = await client.query(
      'UPDATE work_time_records SET status = $1 WHERE wtr_id = $2 RETURNING *',
      [status, wtrId]
    );
    client.release();
